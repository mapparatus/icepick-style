...
class IcepickStyle {
  ...
  constructor (inputStyle, opts={}) {
    ...
    this.current = icepick.freeze(inputStyle || DEFAULT_STYLE);

    VALID_HOOKS.forEach((key) => {
      if (opts.hooks && opts.hooks[key]) {
  ...
  removeHook (key, fn) {
    this._hooks[key] = this._hooks[key].filter((hook) => {
      return (_fn === fn);
    })
  }
